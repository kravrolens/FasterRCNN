...
from models import FasterRCNN, AnchorsGenerator
...
def create_model(num_classes):
    ...
    backbone = MobileNetV2(weights_path="./pretrained/mobilenet_v2.pth").features
...
def main(args):
    ...
    # 用来保存coco_info的文件
    results_file = "outputs/results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))

    # 检查保存权重文件夹是否存在，不存在则创建
    if not os.path.exists("outputs/save_weights"):
        os.makedirs("outputs/save_weights")
    ...
    VOC_root = args.data_root  # VOCdevkit
    aspect_ratio_group_factor = args.aspect_ratio_group_factor
    batch_size = args.batch_size
    amp = args.amp  # 是否使用混合精度训练，需要GPU支持
    ...
    torch.save(model.state_dict(), "./outputs/save_weights/pretrain.pth")
    ...
    for epoch in range(init_epochs, num_epochs + init_epochs, 1):
        ...
        if epoch in range(num_epochs + init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./outputs/save_weights/mobile-model-{}.pth".format(epoch))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_root', required=True, default='./', help="VOC dataset path")
    parser.add_argument('--aspect_ratio_group_factor', type=int, default=3)
    parser.add_argument('--batch_size', type=int, default=8, help='input batch size')
    parser.add_argument('--amp', default=False, help='Mixed Precision Training')
